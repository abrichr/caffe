...
int main(int argc, char** argv) {
  ...
  int new_height = atoi(argv[5]);
  int new_width = atoi(argv[6]);
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
  	std::cout << "Reading " << lines[line_id].first << std::endl;
  	std::cout.flush();
    if (!ReadAndResizeImageToDatum(root_folder + lines[line_id].first,
                          lines[line_id].second, new_height, new_width,
                          &datum)) {
      continue;
    }
